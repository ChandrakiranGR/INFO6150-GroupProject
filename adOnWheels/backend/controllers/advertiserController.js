...
exports.createAd = async (req, res) => {
    try {
        ...
        if (!title || !description || !startDate || !endDate || !budget) {
            return res.status(400).json({ success: false, message: 'All fields (title, description, startDate, endDate, budget) are required.' });
        }
...
exports.getProposals = async (req, res) => {
    try {
        ...
        if (!proposals || proposals.length === 0) {
            return res.status(404).json({ success: false, message: 'No proposals found.' });
        }
...
exports.updateProposal = async (req, res) => {
    try {
        ...
        const proposal = await Proposal.findOne({ _id: proposalId, advertiserId: req.user.id });
        if (!proposal) {
            return res.status(404).json({
                success: false,
                message: 'Proposal not found or you are not authorized to update this proposal.',
            });
        }
